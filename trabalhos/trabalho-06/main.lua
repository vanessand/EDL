...
-- Trabalho 06 --
-- Enumeração: love.keyboard.isDown
  ...
  elseif ball.y > love.graphics.getHeight() - 10 then
	...
	ball.angle = -ball.angle
	score = score-200
	love.graphics.printf(up[1],700,580,800,"left")
  end
  ...
  if collides(ball.x, ball.y, ball.r, ball.r, player.x, player.y, player.w, player.h) then
	...
	oldScore = score
	score = score + 100
	if score > records[index] then
		index = index + 1
		table.insert(records, index, score)
	end
	love.graphics.printf(up[0],700,580,800,"left")
  end
